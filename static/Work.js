...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function populateCertification(cert) {
    ...
    const rows = document.querySelectorAll('#uploadCertTable tbody tr');
    const fileBases = [cert.file_soldering, cert.file_screwing, cert.file_msa]; // DB fields
    rows.forEach((row, idx) => {
      ...
      if (docNo) docNo.value = cert.document_number ?? '';
      if (train) {
        train.value = cert.training_date ?? '';
        train.readOnly = true;
      }
      if (expire) {
        expire.value = cert.expired_date ?? '';
      ...
      // Hide upload controls (IAB view-only)
      if (uploadBtn) { uploadBtn.disabled = true; uploadBtn.classList.add('d-none'); }
      if (hiddenFile) hiddenFile.classList.add('d-none');

      // Attach "view" to viewBtn if file present
      const base64 = fileBases[idx];
      if (viewBtn) {
  ...
  async function searchOperator() {
    ...
    try {
      ...
      if (op.contract_status && String(op.contract_status).toLowerCase() === 'permanen') {
        safeSetValue(endDateInput, '-');
      } else {
...
// =============== Upload and Generate Evaluation Number ===============
let operatorData = null // ini diisi dari proses pencarian NIK yang sudah kamu buat
let uploadedFiles = {
        trainingEval: "",
        skillsEval: "",
        trainingInEval: ""
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const evaluationNumberField = document.getElementById("evaluationNumber");
...
fileTrainingInEval.addEventListener("change", e => {
    ...
    //fileTrainingEval.addEventListener("change", e => uploadedFiles.trainingEval = e.target.files[0] || null);
    //fileSkillsEval.addEventListener("change", e => uploadedFiles.skillsEval = e.target.files[0] || null);
    //fileTrainingInEval.addEventListener("change", e => uploadedFiles.trainingInEval = e.target.files[0] || null);
    ...
    generateBtn.addEventListener("click", () => {
        ...
        if (!uploadedFiles.trainingEval || !uploadedFiles.skillsEval || !uploadedFiles.trainingInEval) {
            alert("Please upload all three required documents before generating the evaluation number.");
            return;
        }
        if (!operatorData.Line || !operatorData.NIK) {
            alert("Line and NIK must be available to generate the evaluation number.");
            return;
        }
        ...
        const evalNumber = `PD-IAB-${operatorData.Line}-${operatorData.NIK}-${dateStr}`;
        evaluationNumberField.textContent = evalNumber;
        //alert("Evaluation number generated successfully.");
    });
    ...
    saveBtn.addEventListener("click", async () => {
        ...
        if (!evaluationNumberField.textContent.trim()) {
            alert("Please generate the evaluation number before saving.");
            return;
        }
        ...
        const res = await fetch("/api/evaluation/save", {
            ...
            body: JSON.stringify({
                ...
                eval_number: evaluationNumberField.textContent,
                file_op_train_eval: base64TrainEval,
                file_op_skills_eval: base64SkillsEval,
                file_train_eval: base64OpTrainEval,
              })
        });
